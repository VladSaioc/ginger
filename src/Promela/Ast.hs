...
data Spec = Spec [Module] deriving (Eq, Ord, Read, Show)

data Module = TopDecl Ident Type Val
  | Init [Pos Stmt]
...
data Stmt = Decl Ident Type (Maybe Exp)
...
data Range = Between Ident Exp Exp
  | In Ident Ident
  deriving (Eq, Ord, Read, Show)
