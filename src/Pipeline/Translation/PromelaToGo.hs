  ...
  Pos p s : ss -> do
    ...
    case s of
      P.As (P.Var x) e -> do
        ...
        translateStatements (ss >: ctx <: (P'.As x' e' : curr ctx))
      ...
      P.Break -> translateStatements ([] >: ctx <: (P'.Break : curr ctx))
      -- Can discard the continuation of 'goto stop_process', since it is unreachable.
      P.Goto "stop_process" -> translateStatements ([] >: ctx <: (P'.Return : curr ctx))
      ...
      P.If os mels ->
        let -- First try to check whether the if statement is a select statement
            ...
            transformToSelect ctx' = case curr ctx' of
              P'.Select cs def ->
                let addCommCase op c ss' = do
                      ctx'' <- translateStatements (ss' >: ctx' <: [])
                      let select = P'.Select ((op c, curr ctx'') : cs) def
                      wrapCtx $ ctx'' <: select
                 in \case
                      ...
                      Pos _ (P.ExpS (P.Const (P.VBool True))) : ss' -> do
                        ctx'' <- translateStatements (ss' >: ctx' <: [])
                        let select = P'.Select ((P'.Star, curr ctx'') : cs) def
                        wrapCtx $ ctx'' <: select
            ...
            transformToIf ctx' =
              let ifSoFar = curr ctx'
               in \case
                    Pos _ (P.ExpS e) : ss' -> do
                      e' <- translateExp (varenv ctx') e
                      ctx'' <- translateStatements (ss' >: ctx' <: [])
                      let ss'' = curr ctx''
                      wrapCtx $ ctx'' <: P'.If e' ss'' [ifSoFar]
         ...
         in case foldM transformToSelect (() >: ctx <: freshSelect) os of
              Bad msg -> do
                ...
                let els = curr ctx'
                ctx'' <- foldM transformToIf (() >: ctx' <: P'.Block els) os
                let ifStmt = flipIfs $ curr ctx''
                translateStatements $ ss >: ctx'' <: (ifStmt : curr ctx)
              Ok ctx' -> case curr ctx' of
                P'.Select cs def -> do
                  let select' = P'.Select (reverse cs) def
                  translateStatements $ ss >: ctx' <: (select' : curr ctx)
                _ -> err "Impossible case. Select transform did not produce select statement."
      P.Decl x t me ->
        let x' = cchain ctx ++ "_" ++ x
              ...
              P.TChan ->
                case me of
                  Just (P.Chan e) -> do
                    ...
                    let chdecl = P'.Chan x' e'
                    translateStatements $ ss >: ctx' <: (chdecl : curr ctx')
        ...
        case M.lookup f (cg ctx) of
          Just (P.Proc _ ps ss') -> do
            ...
            let f' = case M.lookup f $ calls ctx of
                  Just n -> f ++ show n
                  Nothing -> f ++ "0"
            let chainPrefix = cchain ctx ++ "_" ++ f'
            let pes = zip ps es
                addVarInit ((x, t), e) =
                  let x' = chainPrefix ++ "_" ++ x
                      addExp = translateExp (varenv ctx) >=> return . (: []) . P'.As x'
                   in case t of
                        P.TChan -> return []
                ...
                addVarName ve (x, t) =
                  case t of
                    P.TInt -> M.insert x (chainPrefix ++ "_" ++ x) ve
                    P.TBool -> M.insert x (chainPrefix ++ "_" ++ x) ve
                    _ -> ve

            let addCh ce ((a, t), e) =
                  case (t, e) of
                    (P.TChan, P.EVar (P.Var c)) -> M.insert a (chainPrefix ++ "_" ++ c) ce
                    _ -> ce
      ...
      P.For r ss' -> do
        ...
        ctx' <- translateStatements (ss' >: ctx <: [])
        let ctx'' = ctx <: (P'.For x e1' e2' P'.Inc (curr ctx') : curr ctx)
        translateStatements (ss >: ctx'')
      -- P.Do [s] Nothing ->
      P.Do _ _ -> err "Unexpected 'do' statement with non-deterministic branches."

translateRange :: M.Map String String -> P.Range -> Err (String, P'.Exp, P'.Exp)
...
translateExp :: M.Map String String -> P.Exp -> Err P'.Exp
translateExp venv =
   ...
   in \case
        ...
        P.EVar (P.Var x) ->
          case M.lookup x venv of
            ...
            Nothing -> Bad ("[INVALID VARIABLE] binding not found for: " ++ x)
        _ -> Bad "Unexpected expression translation"

translateOp :: Ctxt (Pos P.Stmt) a -> Err (Ctxt () P'.Stmt)
translateOp ctx =
  let translate cons c =
        case M.lookup c (chenv ctx) of
          Just c' -> wrapCtx (ctx <: P'.Atomic (cons c'))
