...
generateSMTLibStmt b (If be te (Just fe)) = 
  SBin SAnd 
  (mkSExpImp (generateSMTLib be) (generateSMTLibStmt b te))
  (SBin SOr (generateSMTLib be) (generateSMTLibStmt b fe))
...
genSMTLibProg prog = 
  ...
  in unpack $ 
     replace "T0" "2" $ -- nasty hacks for now
     replace "T1" "3" $
     replace "T2" "5" $
     pack $ 
     intercalate "\n\n" [stdhead, invdecl, stddcl, predecl, transdecl, postdecl, stdfoot]